...
import { iconMarkers } from './iconsMarkers';
...
const predefinedMarkers: MarkerData[] = [
  ...
  },
  {
    ...
    purchased: false,
    description:
      'A favored location for its shade and the nearby well.'
  },
  {
    position: [1756, 2219],
    iconName: 'prestataire',
...
export function addPredefinedMarkers(map: L.Map, markers: L.Marker[], onMarkerClick?: (data: MarkerData) => void) {
 predefinedMarkers.forEach(markerData => {
   const marker = L.marker(markerData.position, { icon: iconMarkers[markerData.iconName] })
     .addTo(map)
