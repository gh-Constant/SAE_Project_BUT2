    ...
    <div class="py-16">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-12 text-center">
          ...
          <p class="text-base font-body text-stone-grey">{{ t('admin.manage_users.subtitle') }}</p>
        </div>
        ...
        <div>
          <h2 class="text-2xl font-medieval font-bold text-iron-black mb-6 flex items-center">
            ...
            {{ t('admin.manage_users.section_title') }}
          </h2>

          <div v-if="unverifiedUsers.length !== 0"
            class=" bg-white/40 rounded-lg p-8 text-center border border-antique-bronze/10">
            <p class="text-stone-grey italic">{{ t('admin.manage_users.pending_count', { count: unverifiedUsers.length }) }}</p>
          </div>
                ...
                <thead class="bg-antique-bronze/10">
                  <tr>
                    <th scope="col"
                      ...
                      {{ t('admin.manage_users.table.name') }}</th>
                    <th scope="col"
                      class="px-6 py-4 text-left text-xs font-medieval font-bold text-iron-black uppercase tracking-wider">
                      {{ t('admin.manage_users.table.email') }}</th>
                    <th scope="col"
                      class="px-6 py-4 text-left text-xs font-medieval font-bold text-iron-black uppercase tracking-wider">
                      {{ t('admin.manage_users.table.type') }}</th>
                    <th scope="col"
                      class="px-6 py-4 text-center text-xs font-medieval font-bold text-iron-black uppercase tracking-wider">
                      {{ t('admin.manage_users.table.status') }}</th>
                    <th scope="col"
                      class="px-6 py-4 text-center text-xs font-medieval font-bold text-iron-black uppercase tracking-wider">
                      {{ t('admin.manage_users.table.actions') }}</th>
                  </tr>
                </thead>
                  ...
                  <tr v-for="user in allUsers" :key="user.id" class="hover:bg-antique-bronze/5 transition-colors">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-center">
                      <span :class="[
                        ...
                        {{ user.isVerified ? t('admin.manage_users.status.verified') : t('admin.manage_users.status.pending') }}
                      </span>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                      <div class="flex items-center justify-center gap-2">
                        <button v-if="!user.isVerified" @click="verifyUser(user.id)"
                          class="text-green-600 hover:text-green-900 transition-colors p-1" :title="t('admin.manage_users.actions.verify')">
                          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                          </svg>
                        </button>
                        <button @click="deleteUser(user.id)"
                          class="text-red-600 hover:text-red-900 transition-colors p-1" :title="t('admin.manage_users.actions.delete')">
                          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
...
const getRoleLabel = (role: string) => {
  switch (role) {
    case 'prestataire': return t('admin.user_stats.roles.prestataire');
    case 'aventurier': return t('admin.user_stats.roles.aventurier');
    case 'admin': return t('admin.user_stats.roles.admin');
    default: return role;
  }
};
...
const deleteUser = async (userId: number) => {
  if (confirm(t('admin.manage_users.confirm_delete'))) {
