  ...
  <div class="min-h-screen bg-parchment pt-28 pb-24">
    <div class="max-w-4xl mx-auto px-4">
      <div class="text-center mb-10">
        <MedievalSectionTitle>{{ t('home.leaderboard.title') }}</MedievalSectionTitle>
        <p class="text-stone-grey font-body mt-2">
          {{ t('home.leaderboard.subtitle') }}
        </p>
      </div>

      <div class="bg-parchment rounded-lg shadow-2xl border-4 border-double border-antique-bronze overflow-hidden relative">
        ...
        <table class="w-full text-left">
          <thead class="bg-antique-bronze/10 text-dark-wood font-medieval uppercase text-sm tracking-wider border-b-2 border-antique-bronze/30">
            <tr>
              <th class="p-5 w-20 text-center">{{ t('home.leaderboard.columns.rank') }}</th>
              <th class="p-5">{{ t('home.leaderboard.columns.adventurer') }}</th>
              <th class="p-5 w-32 text-center">{{ t('home.leaderboard.columns.level') }}</th>
              <th class="p-5 w-40 text-right">{{ t('home.leaderboard.columns.xp') }}</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-antique-bronze/20 font-body">
            ...
            <tr v-for="user in leaderboard" v-else :key="user.id"
              ...
              :class="{ 'bg-amber-100/60 border-y-2 shadow-sm relative z-10 font-bold': auth.user && user.id === auth.user.id }">
              <td class="p-4 text-center font-bold text-dark-wood">
                <div v-if="user.rank <= 3" class="flex items-center justify-center w-10 h-10 mx-auto rounded-full shadow-sm"
                  :class="user.rank === 1 ? 'bg-yellow-100 text-yellow-700 ring-2 ring-yellow-400' : user.rank === 2 ? 'bg-gray-100 text-gray-700 ring-2 ring-gray-400' : 'bg-orange-100 text-orange-800 ring-2 ring-orange-400'">
                  <span class="text-xl">{{ user.rank === 1 ? '🥇' : user.rank === 2 ? '🥈' : '🥉' }}</span>
                </div>
                <span v-else class="font-medieval text-lg text-stone-grey/80 font-bold">#{{ user.rank }}</span>
              </td>
              <td class="p-4">
                <div class="flex items-center gap-3">
                  <div class="w-10 h-10 rounded-full border border-antique-bronze/50 overflow-hidden bg-warm-sand shadow-sm">
                    <img v-if="user.avatar_url" :src="user.avatar_url" class="w-full h-full object-cover" :alt="t('profile.fields.avatar')" />
                    <div v-else class="w-full h-full flex items-center justify-center bg-antique-bronze/20 text-antique-bronze">
                  ...
                  <div>
                    ...
                    <div class="text-xs text-stone-500 italic">
                      {{ user.bio ? user.bio.substring(0, 30) + (user.bio.length > 30 ? '...' : '') : t('home.leaderboard.mysterious_adventurer') }}
                    </div>
                  </div>
                </div>
              </td>
              <td class="p-4 text-center">
                <span class="inline-block px-3 py-1 rounded-full bg-antique-bronze/10 text-dark-wood font-bold border border-antique-bronze/20 shadow-sm text-sm">
                  {{ t('home.leaderboard.level_short', { level: user.level }) }}
                </span>
              </td>
              <td class="p-4 text-right">
                <span class="font-bold text-dark-wood/80">{{ formatXp(user.xp) }}</span>
                <span class="text-xs text-stone-500 uppercase ml-1">{{ t('home.leaderboard.columns.xp') }}</span>
              </td>
            </tr>
          </tbody>
        </table>

        <div v-if="!loading && leaderboard.length === 0" class="p-16 text-center text-stone-grey italic font-medieval text-lg">
          <i class="fas fa-scroll text-4xl mb-4 text-antique-bronze/40"></i>
          <p>{{ t('home.leaderboard.empty') }}</p>
        </div>
      </div>
      ...
      <div v-if="totalPages > 1" class="flex justify-center items-center gap-6 mt-8">
        <button @click="changePage(currentPage - 1)" :disabled="currentPage === 1"
          ...
          <span>{{ t('home.leaderboard.previous') }}</span>
        </button>

        <span class="font-medieval text-dark-wood text-lg px-4 py-2 bg-white/50 rounded border border-stone-200 shadow-inner">
          {{ t('home.leaderboard.page') }}
          <span class="font-bold text-antique-bronze">{{ currentPage }}</span>
          <span class="text-sm text-stone-400">/ {{ totalPages }}</span>
        </span>

        <button @click="changePage(currentPage + 1)" :disabled="currentPage === totalPages"
          class="group flex items-center gap-2 px-5 py-2 bg-parchment border-2 border-antique-bronze rounded-lg text-dark-wood hover:bg-antique-bronze hover:text-white disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-parchment disabled:hover:text-dark-wood transition-all duration-300 font-medieval shadow-md">
          <span>{{ t('home.leaderboard.next') }}</span>
      ...
      <transition enter-active-class="transform transition ease-out duration-300"
        ...
        <div v-if="auth.user && !isCurrentUserVisible && !loading"
          ...
          <div class="flex items-center gap-5 relative z-10 transition-transform hover:scale-105">
            <div class="relative">
              <div class="bg-gradient-to-br from-gold to-amber-600 text-dark-wood font-bold rounded-full w-14 h-14 flex items-center justify-center border-4 border-dark-wood shadow-lg">
                <span class="font-medieval text-xl">#{{ userRank }}</span>
              </div>
              <div class="absolute -bottom-1 -right-1 bg-dark-wood text-gold text-[10px] px-1.5 py-0.5 rounded border border-gold/30 uppercase font-bold">
                {{ t('home.leaderboard.columns.rank') }}
              </div>
            </div>
            <div class="flex flex-col">
              <span class="font-medieval font-bold text-xl leading-none text-gold drop-shadow-sm">{{ t('home.leaderboard.sticky_title') }}</span>
              <span class="text-xs text-stone-300/80 mt-1">{{ t('home.leaderboard.sticky_subtitle') }}</span>
            </div>
          </div>

          <div class="flex items-center gap-8 relative z-10">
            <div class="text-center group cursor-default">
              <div class="text-[10px] uppercase text-stone-400 font-bold tracking-widest mb-0.5 group-hover:text-gold transition-colors">
                {{ t('home.leaderboard.columns.level') }}
              </div>
