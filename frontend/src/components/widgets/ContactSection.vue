<template>
  <div class="contact-section" v-if="authStore.isAuthenticated">
    <div class="flex justify-between items-center mb-4">
      <h3 class="text-xl font-bold text-gray-800">{{ t('widgets.contact.title') }}</h3>
    </div>
    ...
    <div v-if="successMessage" class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded relative mb-4" role="alert">
      <strong class="font-bold">{{ t('widgets.contact.success_title') }}</strong>
      <span class="block sm:inline">{{ successMessage }}</span>
      <span class="absolute top-0 bottom-0 right-0 px-4 py-3" @click="successMessage = ''">
        <svg class="fill-current h-6 w-6 text-green-500" role="button" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><title>{{ t('widgets.contact.close_alert') }}</title><path d="M14.348 14.849a1.2 1.2 0 0 1-1.697 0L10 11.819l-2.651 3.029a1.2 1.2 0 1 1-1.697-1.697l2.758-3.15-2.759-3.152a1.2 1.2 0 1 1 1.697-1.697L10 8.183l2.651-3.031a1.2 1.2 0 1 1 1.697 1.697l-2.758 3.152 2.758 3.15a1.2 1.2 0 0 1 0 1.698z" /></svg>
      </span>
    </div>
    ...
    <form @submit.prevent="sendMessage" class="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
      <div class="mb-6">
        <label class="block text-gray-700 text-sm font-bold mb-2" for="message">
          {{ t('widgets.contact.message_label') }}
        </label>
        <textarea
          ...
          id="message"
          :placeholder="t('widgets.contact.message_placeholder')"
          required
        ></textarea>
      </div>
      <div class="flex items-center justify-end">
        ...
        >
          ...
          {{ isSending ? t('widgets.contact.sending_button') : t('widgets.contact.send_button') }}
        </button>
      </div>
    </form>
  </div>
...
const sendMessage = async () => {
  ...
  try {
    ...
    successMessage.value = t('widgets.contact.success_message');
    formData.message = '';
    ...
    alert(t('widgets.contact.error_message'));
  } finally {
