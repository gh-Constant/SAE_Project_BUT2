<template>
  <div class="quiz-card bg-white rounded-xl shadow-md overflow-hidden hover:shadow-lg transition-shadow cursor-pointer"
    @click="$emit('click')">
    <!-- Quiz Image -->
    <div class="h-40 bg-antique-bronze relative">
      <img v-if="quiz.image_url" :src="quiz.image_url" :alt="quiz.title" class="w-full h-full object-cover" />
      ...
      <div class="absolute top-3 right-3 bg-black/50 text-white px-2 py-1 rounded-full text-xs font-medium">
        {{ $t('quiz.list.questions_count', { count: questionCount }) }}
      </div>

      <!-- Inactive badge -->
      <div v-if="!quiz.is_active"
        class="absolute top-3 left-3 bg-red-500 text-white px-2 py-1 rounded-full text-xs font-medium">
        {{ $t('quiz.list.inactive') }}
      </div>

      <!-- Edit Button (FAB style) -->
      <button v-if="canEdit" @click.stop="$emit('edit')"
        ...
        :title="$t('quiz.list.edit_tooltip')">
    ...
    <div class="p-4">
      ...
      <div class="flex items-center justify-between text-xs text-stone-grey/60">
        <span v-if="quiz._count?.attempts !== undefined" class="flex items-center">
          ...
          {{ $t('quiz.list.attempts_count', { count: quiz._count.attempts }) }}
        </span>
        <span v-if="quiz.creator">
          {{ $t('quiz.list.creator', { name: quiz.creator.firstname }) }}
        </span>
      </div>
    </div>
  </div>
