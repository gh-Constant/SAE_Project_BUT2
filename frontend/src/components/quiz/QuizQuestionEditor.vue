<template>
  <div class="quiz-question-editor bg-white rounded-xl shadow-md p-6 mb-4">
    ...
    <div class="flex items-center justify-between mb-4">
      ...
      <button v-if="canDelete" class="text-red-500 hover:text-red-700 transition-colors p-1" @click="$emit('delete')"
    ...
    <div class="mb-4">
      ...
      <Editor ref="editorRef" :initialContent="modelValue.content" @ready="onEditorReady" />
    </div>
    ...
    <div class="mb-4">
      ...
      <div class="flex items-center gap-4">
        <!-- Preview -->
        <div v-if="modelValue.image_url"
          class="relative group w-24 h-16 rounded-lg overflow-hidden border border-stone-grey/20">
          <img :src="modelValue.image_url" class="w-full h-full object-cover" alt="Preview" />
          <button type="button" @click="updateField('image_url', '')"
        ...
        <div class="flex-1">
          <label
            ...
            <div class="flex items-center gap-2 text-stone-grey">
              ...
              <span class="text-sm">{{ uploading ? $t('quiz.editor.uploading') : $t('quiz.editor.choose_image')
                }}</span>
            </div>
    ...
    <div class="mb-4">
      <div class="flex items-center justify-between mb-2">
        ...
        <button v-if="modelValue.answers.length < 6"
          class="text-antique-bronze hover:text-amber-700 text-sm flex items-center" @click="addAnswer">
      ...
      <div class="space-y-2">
        <div v-for="(answer, answerIndex) in modelValue.answers" :key="answerIndex" class="flex items-center gap-2">
          <!-- Correct answer radio -->
          <button class="flex-shrink-0 w-6 h-6 rounded-full border-2 flex items-center justify-center transition-colors"
            :class="{
              'border-emerald-500 bg-emerald-500': answer.is_correct,
              'border-stone-grey/40 hover:border-emerald-400': !answer.is_correct,
            }" @click="toggleCorrectAnswer(answerIndex)" :title="$t('quiz.editor.question.toggle_correct')">
          ...
          <!-- Delete answer -->
          <button v-if="modelValue.answers.length > 2"
            class="flex-shrink-0 text-red-400 hover:text-red-600 transition-colors" @click="removeAnswer(answerIndex)"
