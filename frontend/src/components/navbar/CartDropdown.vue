  ...
  <div
    ...
    <div class="max-h-80 overflow-y-auto custom-scrollbar">
      ...
      <div v-else class="divide-y divide-antique-bronze/10">
        <div v-for="item in items" :key="item.id_product"
          class="px-6 py-4 hover:bg-antique-bronze/5 transition-colors group">
          <div class="flex gap-4">
            <!-- Image -->
            <div
              class="w-20 h-20 rounded-sm bg-warm-sand overflow-hidden flex-shrink-0 border border-antique-bronze/30 shadow-inner">
              <img v-if="getProduct(item)?.image" :src="getProduct(item)?.image" :alt="getProduct(item)?.name"
            ...
            <div class="flex-1 min-w-0 flex flex-col justify-between py-1">
              ...
              <div class="flex justify-between items-end">
                ...
                <button @click.stop="removeItem(item.id_product)"
    ...
    <div v-if="items.length > 0" class="px-6 pt-4 border-t-2 border-antique-bronze/20 bg-antique-bronze/5">
      <div class="flex justify-between items-center mb-5">
        ...
        <span class="font-medieval text-iron-black text-2xl">{{ $t('cart.price_format', { price: total.toFixed(2) })
          }}</span>
      </div>
