  ...
  <nav class="fixed top-0 left-0 right-0 z-[1100] bg-transparent">
        ...
        <div class="flex items-center space-x-4 ml-auto">
          ...
          <div v-if="isLoggedIn && isAventurier(auth.user)" class="relative cart-dropdown-container hidden md:block">
            ...
            <transition enter-active-class="transition ease-out duration-200" enter-from-class="opacity-0 translate-y-1"
              ...
              <CartDropdown v-if="showCartDropdown" @close="showCartDropdown = false" />
            </transition>
          </div>
          ...
          <div v-if="isLoggedIn" class="relative dropdown-container hidden md:block">
            ...
            <div v-if="showDropdown"
              ...
              <router-link v-if="isPrestataire(auth.user)" to="/prestataire"
                class="flex items-center w-full px-4 py-2 text-sm font-medieval text-dark-wood hover:bg-antique-bronze/10 transition-colors"
                @click="closeDropdown">
              ...
              <router-link v-if="isAventurier(auth.user)" to="/commandes"
                class="flex items-center w-full px-4 py-2 text-sm font-medieval text-dark-wood hover:bg-antique-bronze/10 transition-colors"
                @click="closeDropdown">
                <i class="fas fa-list mr-3 text-antique-bronze" />
                {{ $t('navbar.my_orders') }}
              </router-link>
              <router-link v-if="isAventurier(auth.user)" to="/my-reservations"
                class="flex items-center w-full px-4 py-2 text-sm font-medieval text-dark-wood hover:bg-antique-bronze/10 transition-colors"
                @click="closeDropdown">
                <i class="fas fa-ticket-alt mr-3 text-antique-bronze" />
                {{ $t('navbar.my_reservations') }}
              </router-link>
              <router-link v-if="isAventurier(auth.user)" to="/my-quests"
                class="flex items-center w-full px-4 py-2 text-sm font-medieval text-dark-wood hover:bg-antique-bronze/10 transition-colors"
                @click="closeDropdown">
                <i class="fas fa-scroll mr-3 text-antique-bronze" />
                {{ $t('navbar.my_quests') }}
              </router-link>
              <router-link v-if="isAventurier(auth.user)" to="/leaderboard"
                class="flex items-center w-full px-4 py-2 text-sm font-medieval text-dark-wood hover:bg-antique-bronze/10 transition-colors"
                @click="closeDropdown">
          ...
          <div v-if="isDev" class="relative dev-dropdown-container hidden md:block">
            <MedievalButton @click="toggleDevDropdown" small variant="transparent" class="!px-3" title="Dev Tools">
              <i class="fas fa-chevron-down text-xs"></i>
            </MedievalButton>

            <!-- Dropdown Menu -->
            <div v-if="showDevDropdown"
              class="absolute right-0 mt-2 w-48 bg-parchment rounded-sm shadow-xl py-2 z-50 border-2 border-antique-bronze">
              <div
                class="px-4 py-2 border-b border-antique-bronze/20 text-xs font-bold text-stone-grey uppercase tracking-wider font-medieval">
                Quick Login (Dev)
              </div>
              <button
                class="flex items-center w-full px-4 py-2 text-sm font-medieval text-dark-wood hover:bg-antique-bronze/10 transition-colors"
          ...
          <div class="flex md:hidden">
            <MedievalButton @click="toggleMobileMenu" class="!p-2 !shadow-[0_2px_0_#5D4037] !active:translate-y-[2px]">
    ...
    <transition enter-active-class="transition duration-200 ease-out"
      ...
      <div v-if="isMobileMenuOpen"
        class="md:hidden bg-parchment border-b-4 border-antique-bronze shadow-xl max-h-[calc(100vh-5rem)] overflow-y-auto">
        <div class="px-4 pt-2 pb-6 space-y-3">
          <MedievalButton to="/" full-width class="!justify-start !shadow-[0_2px_0_#5D4037] !active:translate-y-[2px]"
            active-class="!bg-[#8B6B43] !border-[#5D4037]" @click="isMobileMenuOpen = false">
            <i class="fas fa-home text-lg w-8"></i>
            <span class="ml-2">{{ $t('navbar.home') }}</span>
          </MedievalButton>

          <MedievalButton v-if="isLoggedIn" to="/map" full-width
          ...
          <MedievalButton v-if="isLoggedIn && isAventurier(auth.user)" to="/events" full-width
            class="!justify-start !shadow-[0_2px_0_#5D4037] !active:translate-y-[2px]"
            active-class="!bg-[#8B6B43] !border-[#5D4037]" @click="isMobileMenuOpen = false">
          ...
          <MedievalButton v-if="isLoggedIn && isAventurier(auth.user)" to="/leaderboard" full-width
            class="!justify-start !shadow-[0_2px_0_#5D4037] !active:translate-y-[2px]"
            active-class="!bg-[#8B6B43] !border-[#5D4037]" @click="isMobileMenuOpen = false">
          ...
          <MedievalButton v-if="isLoggedIn && isAventurier(auth.user)" to="/panier" full-width
            class="!justify-start !shadow-[0_2px_0_#5D4037] !active:translate-y-[2px]"
          ...
          <template v-if="isLoggedIn">
            <div class="flex items-center px-2 py-2 mb-2">
              <div
                class="w-10 h-10 bg-gradient-to-br from-orange-500 to-amber-600 rounded-full flex items-center justify-center overflow-hidden border border-white/30 shadow-md mr-3">
                <img v-if="auth.user?.avatar_url" :src="auth.user.avatar_url" :alt="auth.user.firstname"
                  class="w-full h-full object-cover">
            ...
            <MedievalButton v-if="isAventurier(auth.user)" to="/my-reservations" full-width
              class="!justify-start !shadow-[0_2px_0_#5D4037] !active:translate-y-[2px]"
              @click="isMobileMenuOpen = false">
            ...
            <MedievalButton v-if="isAventurier(auth.user)" to="/my-quests" full-width
              class="!justify-start !shadow-[0_2px_0_#5D4037] !active:translate-y-[2px]"
              @click="isMobileMenuOpen = false">
            ...
            <MedievalButton to="/conversations" full-width
              class="!justify-start !shadow-[0_2px_0_#5D4037] !active:translate-y-[2px]"
              @click="isMobileMenuOpen = false">
            ...
            <MedievalButton v-if="isAdmin(auth.user)" to="/admin" full-width
              class="!justify-start !shadow-[0_2px_0_#5D4037] !active:translate-y-[2px]"
              @click="isMobileMenuOpen = false">
            ...
            <MedievalButton v-if="isPrestataire(auth.user)" to="/prestataire" full-width
              class="!justify-start !shadow-[0_2px_0_#5D4037] !active:translate-y-[2px]"
              @click="isMobileMenuOpen = false">
          ...
          <template v-else>
            <MedievalButton to="/login" full-width
              class="!justify-start !shadow-[0_2px_0_#5D4037] !active:translate-y-[2px]"
              @click="isMobileMenuOpen = false">
              <i class="fas fa-sign-in-alt text-lg w-8"></i>
              <span class="ml-2">{{ $t('navbar.signin') }}</span>
            </MedievalButton>
            <MedievalButton to="/register" full-width
